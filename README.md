...
다음 목표는 **손품**입니다. 호갱노노·네이버부동산에서 조건에 맞는 매물을 찾을 때,
매번 필터를 다시 걸고 결과를 하나씩 지도로 열어 지하철·마트·학교가 가까운지 확인하는 반복 작업 말입니다.

- **저장된 검색 바로가기** — 조건별 링크를 저장해두고 한 번에 엽니다
- **인프라 자동 조회** — 매물을 등록하면 반경 안의 지하철역·마트·초등학교·공원을 **공식 API로 자동으로 채웁니다.**
  카카오 로컬, 학교알리미, 소상공인시장진흥공단 상가정보, 브이월드 — 전부 공개 데이터입니다
- **클립보드 붙여넣기** — 매물 화면을 복사해 붙여넣으면 단지명·면적·층·가격을 파싱해 폼을 채웁니다

**"조건을 넣으면 매물을 찾아준다"는 만들지 않습니다.** 실거래가는 정부가 공개하지만
호가 목록은 공개 원천이 없어, 긁어오는 것 외에 방법이 없기 때문입니다.
자세한 판단 근거와 크롤링에 대한 선은 `docs/ROADMAP.md` 에 적어 두었습니다.
